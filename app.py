...
def fetch_thingspeak_data():
    ...
    if 'feeds' in data and data['feeds']:
        entry = data['feeds'][0]
        pressure = entry.get('field1', 'N/A')
        humidity = entry.get('field2', 'N/A')
        water_level = entry.get('field3', 'N/A')
        temperature = entry.get('field4', 'N/A')
        return {
            'pressure': pressure,
            'humidity': humidity,
...
def generate_alerts(thingspeak_data):
    ...
    if water_level and float(water_level) > 2000:
